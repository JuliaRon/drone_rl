...
directions = [np.array([0.,0.,0.]), np.array([1.,0.,0.]), np.array([-1.,0.,0.]), np.array([0.,1.,0.]),
...
class DroneEnv(gym.Env):
  ...
  def step(self, action):
    ...
    if action == 7:
      ...
      reward = DART_RADIUS - np.linalg.norm(dart_center - dart_pos) - (DART_RADIUS - np.linalg.norm(dart_center - np.array([10.,10.,0.])))
      done = True
    else:
    ...
    for i in range(0,11):
      if np.linalg.norm(dart_center - np.append(new_muav_pos[0:2], 0)) < i*2 and not self.intermediateRewards[i]:
        print("crossed over intermed " + str(i))
        reward += 5
        self.intermediateRewards[i] = True
      if np.linalg.norm(dart_center - np.append(new_muav_pos[0:2], 0)) > i*2 and self.intermediateRewards[i]:
  ...
  def reset(self):
    ...
    self.state = (10.,10.,10., 0.,0.,0., 25., 25., 0., MAX, TIME_MAX)
    self.intermediateRewards = [False] * 11
